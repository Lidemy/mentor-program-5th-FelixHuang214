...
import { register, getUser } from "../../WebAPI";
...
import { Droplet as DropletIcon } from "@styled-icons/boxicons-solid";
...
const StyledDropletIcon = styled(DropletIcon)`
  width: 35px;
`;
...
const NicknameInput = ({ desc, value, event }) => {
  const { handleChangeValue } = event;
  return (
    <InputWrapper>
      <IconWrapper>
...
export default function LoginPage() {
  ...
  const { username, password, nickname } = inputValue;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        dispatch({ type: "REGISTER_FAILURE", payload: "帳號重複請重新輸入" });
        return;
      }
